...
export default function KaryawanPage() {
  ...
  const handleSimpanDept = async (e) => {
    ...
    // VALIDASI: Cegah duplikasi kombinasi Nama Departemen + Leader
    const isDuplicate = departemen.find(d => {
       if (deptForm.id && d.id === deptForm.id) return false;
       const cekNama = d.nama_departemen === deptForm.nama_departemen;
       const cekLeader = (d.id_leader || '') === (deptForm.id_leader || '');
       return cekNama && cekLeader;
    });

    if (isDuplicate) { alert('Ditolak: Kombinasi Induk Departemen dan Team Leader tersebut sudah ada dalam daftar.'); return; }
  ...
  const hapusDept = async (id) => {
    if (!confirm('Hapus departemen ini? Karyawan di bawahnya akan kehilangan departemen.')) return;
  ...
  const handleSimpanKaryawan = async (e) => {
    ...
    setLoading(true);
    let payload = { nama_lengkap: form.nama_lengkap, id_departemen: form.id_departemen || null }; 
    
    let error;
    if (editKarId) {
      // Mode Edit: Hanya update Nama dan Departemen (NIP & Username terkunci mutlak)
      const { error: err } = await supabase.from('karyawan').update(payload).eq('id', editKarId); 
      error = err;
    } else {
      // Mode Tambah Baru: Eksekusi Generator NIP (Prefix + Nama + 3 Angka Acak)
      const selectedDept = departemen.find(d => d.id === form.id_departemen);
      const prefix = selectedDept?.nama_departemen === 'Marketing' ? 'MT' : 'BO';
      const cleanName = form.nama_lengkap.trim().replace(/[^a-zA-Z0-9]/g, '').substring(0, 4).toUpperCase();
      const randomStr = Math.floor(100 + Math.random() * 900);
      const generatedId = `${prefix}-${cleanName}${randomStr}`; 

      payload.nip = generatedId;
      payload.username = generatedId.toLowerCase();
  ...
  // Kalkulator Realtime untuk Preview Auto-Generate NIP & Username di UI
  const getPreviewId = () => {
    if (editKarId) return { nip: form.nip, username: form.username }; // Jika edit, tampilkan ID lama
    
    let prefix = 'XX';
    if (form.id_departemen) {
       const selectedDept = departemen.find(d => d.id === form.id_departemen);
       if (selectedDept?.nama_departemen === 'Marketing') prefix = 'MT';
       else if (selectedDept?.nama_departemen === 'Backoffice') prefix = 'BO';
    }
    const cleanName = form.nama_lengkap ? form.nama_lengkap.trim().replace(/[^a-zA-Z0-9]/g, '').substring(0, 4).toUpperCase() : 'XXXX';
    return { nip: `${prefix}-${cleanName}***`, username: `${prefix.toLowerCase()}-${cleanName.toLowerCase()}***` };
  };

  const idPreview = getPreviewId();
    ...
    <div className="space-y-6 relative">
            ...
            <form onSubmit={handleSimpanDept} className={`p-6 border-b border-slate-200 ${deptForm.id ? 'bg-indigo-50/50' : 'bg-slate-50'}`}>
               <div className="space-y-4">
                 <div>
                   ...
                   {/* DIUBAH MENJADI DROPDOWN HANYA 2 OPSI */}
                   <select required value={deptForm.nama_departemen} onChange={e => setDeptForm({...deptForm, nama_departemen: e.target.value})} className={getStyleInput(false)}>
                     <option value="">-- Pilih Induk Departemen --</option>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
              <h2 className="text-lg font-bold text-slate-900">{editKarId ? 'Edit Data Karyawan' : 'Tambah Karyawan Manual'}</h2>
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-rose-500"><X className="w-5 h-5" /></button>
            </div>
            
            <form onSubmit={handleSimpanKaryawan} className="p-6 space-y-5">
              
              {/* DIUBAH: Pilihan Departemen Ditarik ke Posisi Paling Atas */}
              <div>
                ...
                <select value={form.id_departemen} onChange={e => {setForm({...form, id_departemen: e.target.value}); setErrorsKar({...errorsKar, id_departemen: null});}} className={getStyleInput(errorsKar.id_departemen)}>
                  ...
                  {departemen.map(d => {
                     const leader = karyawan.find(k => k.id === d.id_leader);
                     // DIUBAH: Menampilkan format "Marketing (Team Aris)" di dalam dropdown
                     return <option key={d.id} value={d.id}>{d.nama_departemen} {leader ? `(Team ${leader.nama_lengkap})` : ''}</option>
                  })}
                </select>
              ...
              <div>
                ...
                <input type="text" value={form.nama_lengkap} onChange={e => {setForm({...form, nama_lengkap: e.target.value}); setErrorsKar({...errorsKar, nama_lengkap: null});}} className={getStyleInput(errorsKar.nama_lengkap)} placeholder="Contoh: Aris Wijaya"/>
                {errorsKar.nama_lengkap && <p className="text-rose-500 text-[10px] mt-1 italic font-bold">{errorsKar.nama_lengkap}</p>}
              </div>

              {/* DIUBAH: NIP & Username kini di-disable mutlak & muncul Pratinjau Auto-Generate */}
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100">
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-1">NIP (Read-Only)</label>
                  <input type="text" value={idPreview.nip} disabled className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-500 text-sm cursor-not-allowed font-mono shadow-inner"/>
                </div>
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Username (Read-Only)</label>
                  <input type="text" value={idPreview.username} disabled className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-500 text-sm cursor-not-allowed font-mono shadow-inner"/>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={loading} className="px-6 py-2 text-white font-medium bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors">{loading ? 'Proses...' : (editKarId ? 'Simpan Perubahan' : 'Daftarkan Karyawan')}</button>
              </div>
            </form>
          </div>
        </div>
